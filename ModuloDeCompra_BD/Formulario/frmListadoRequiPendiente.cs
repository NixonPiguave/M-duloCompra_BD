...
namespace ModuloDeCompra_BD.Formulario
{
    public partial class frmListadoRequiPendiente: Form
    {
        ...
        {
            dgvListRequiPendientes.DataSource = CsComandosSql.RetornaDatos($"Select ID_Requisicion, Estado_Requisicion, MotivoRequisicion, Nombre_Departamento from (Select * from (Select R.ID_Usuario, U.ID_Depa, R.ID_Requisicion, R.Estado_Requisicion,R.MotivoRequisicion, D.Nombre_Departamento\r\nfrom Requisicion R inner join Usuario U on R.ID_Usuario= U.ID_Usuario inner join Departamento D on U.ID_Depa= D.ID_Depa) as X where ID_Depa in\r\n(Select D.ID_Depa from Departamento D inner join Usuario U on D.ID_Depa= U.ID_Depa where ID_Usuario= {IdUsuario1})) as Y where Estado_Requisicion= 'Pendiente'\r\n");
        }
