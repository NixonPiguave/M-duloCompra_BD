    ...
    public partial class frmReporteDiarioContable : Form
    {
        ...
        private void btnGenerarReporte_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(txtDiarioID.Text))
            {
                ...
                string sentencia = $"select 1 as NUM,YEAR(DH.Fecha) AS ANIO, MONTH(DH.Fecha) AS MES,CONVERT(date, DH.Fecha) AS Fecha,CuentaDebito as Cuenta,\r\n(select Nombre from CatalogoCuentas where Cuenta= DH.CuentaDebito) AS NombreCuenta,\r\n(select Descripcion from CatalogoCuentas where Cuenta= DH.CuentaDebito) as Descripcion, \r\n(select Categoria from CatalogoCuentas where Cuenta= DH.CuentaDebito) as Categoria, DH.Usuario, DH.Fuente, DH.DiarioFuente AS Referencia, DH.Debito, null as Credito, DH.Diario\r\nfrom DiarioCont_Header DH where DH.Diario={ide}\r\nunion \r\nselect 2 as NUM,YEAR(DH.Fecha) AS ANIO, MONTH(DH.Fecha) AS MES,CONVERT(date, DH.Fecha) AS Fecha,CuentaCredito as Cuenta,\r\n(select Nombre from CatalogoCuentas where Cuenta= DH.CuentaCredito) AS NombreCuenta,\r\n(select Descripcion from CatalogoCuentas where Cuenta= DH.CuentaCredito) as Descripcion, \r\n(select Categoria from CatalogoCuentas where Cuenta= DH.CuentaCredito) as Categoria, DH.Usuario, DH.Fuente, DH.DiarioFuente AS Referencia, null as Debito ,DH.Credito, DH.Diario\r\nfrom DiarioCont_Header DH where DH.Diario={ide}";
